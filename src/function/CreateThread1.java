...
public class CreateThread1 extends Thread{
	...
	public CreateThread1(){		init();		}
	
	public CreateThread1(Socket socket,String ip,Integer port){
		this.socket = socket;
		this.ip = ip;
		this.port = port;
		init();
...
//					System.out.println("1号读取数据："+readtext.trim());
	...
	private void string_dispose(String str){
		...
		switch(p1){
		case 1://轴信息
			s.ax1.setType(String.valueOf(p1));
			...
			s.ax1.setA_axis_work(a_axis_work);
			...
			s.ax1.setA_axis_relative(a_axis_relative);
			...
			s.ax1.setA_axis_machine(a_axis_machine);
			...
			s.ax1.setA_axis_remainder(a_axis_remainder);
			...
			s.ax1.setA_f_value(a_f_value);
			
			int q19 = Dao_string(str.substring(152, 160));
			int q20 = Dao_string(str.substring(160, 168));
			s.ax1.setC_s_value(q19);
			s.ax1.setA_s_value(q20);
			
			String source = To_ASCII(str.substring(168, 215));
			s.ax1.setTimestamp(source);
			s.flag1 = true;
			break;
		case 2://报警
			s.al1.setType(String.valueOf(p1));
			s.al1.setAlarmcode(Dao_string(str.substring(8, 16)));
			s.al1.setAlarmtime_occur(To_ASCII(str.substring(16, 64)));
			s.al1.setAlarmtime_remove(Dao_string(str.substring(64, 72)));
			s.al1.setAlarmnum(To_ASCII(str.substring(72, 74)));
			s.al1.setTimestamp(To_ASCII(str.substring(74, 128)));
			...
			s.al1.setAlarmcode_array(alarmcode_array);
			...
			s.al1.setAlarmtime_occur_array(alarmtime_occur_array);
			
			s.flag1 = true;
			break;
		case 3://系统
			s.sy1.setType(String.valueOf(p1));
			s.sy1.setSystemid(To_ASCII(str.substring(2, 18)));
			s.sy1.setSystemtype(To_ASCII(str.substring(18, 50)));
			s.sy1.setSystemver(To_ASCII(str.substring(50, 82)));
			s.sy1.setGgroup(To_ASCII(str.substring(82, 162)));
			s.sy1.setPn(To_ASCII(str.substring(162, 192)));
			
			s.sy1.setPd(Integer.valueOf(Dao_string(str.substring(192, 200))));
			s.sy1.setPs(Integer.valueOf(Dao_string(str.substring(200, 208))));
			s.sy1.setPl(Integer.valueOf(Dao_string(str.substring(208, 216))));
			
			s.sy1.setS_loadcurrent(Dao_string(str.substring(216, 224)));
			...
			s.sy1.setAxis_loadcurrent(axis_loadcurrent);
			
			s.sy1.setOntime(Dao_string(str.substring(248, 256)));
			s.sy1.setRuntime(Dao_string(str.substring(256, 264)));
			s.sy1.setTimestamp(To_ASCII(str.substring(264, 312)));
