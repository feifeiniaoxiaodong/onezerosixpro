...
public class CreateThread4 extends Thread{
	...
	public CreateThread4(){		init();		}
	
	public CreateThread4(Socket socket,String ip,Integer port){
		this.socket = socket;
		this.ip = ip;
		this.port = port;
		init();
...
//					System.out.println("4号读取数据："+readtext.trim());
...
//		int i = (int)Long.parseLong(ss, 16);
	...
	private void string_dispose(String str){
		...
		switch(p1){
		case 1://轴信息
			s.ax4.setType(String.valueOf(p1));
			...
			s.ax4.setA_axis_work(a_axis_work);
			...
			s.ax4.setA_axis_relative(a_axis_relative);
			...
			s.ax4.setA_axis_machine(a_axis_machine);
			...
			s.ax4.setA_axis_remainder(a_axis_remainder);
			...
			s.ax4.setA_f_value(a_f_value);
			
			int q19 = Dao_string(str.substring(152, 160));
			int q20 = Dao_string(str.substring(160, 168));
			s.ax4.setC_s_value(q19);
			s.ax4.setA_s_value(q20);
			
			String source = To_ASCII(str.substring(168, 215));
			s.ax4.setTimestamp(source);
			s.flag4 = true;
			break;
		case 2://报警
			s.al4.setType(String.valueOf(p1));
			s.al4.setAlarmcode(Dao_string(str.substring(8, 16)));
			s.al4.setAlarmtime_occur(To_ASCII(str.substring(16, 64)));
			s.al4.setAlarmtime_remove(Dao_string(str.substring(64, 72)));
			
			String s0 = str.substring(72, 74);
			BigInteger i0 = new BigInteger(s0,16);
			int j0 = i0.intValue();
			s.al4.setAlarmnum(String.valueOf(j0));
			s.al4.setTimestamp(To_ASCII(str.substring(74, 128)));
			...
			s.al4.setAlarmcode_array(alarmcode_array);
			...
			s.al4.setAlarmtime_occur_array(alarmtime_occur_array);
			
			s.flag4 = true;
			break;
		case 3://系统
			s.sy4.setType(String.valueOf(p1));
			s.sy4.setSystemid(To_ASCII(str.substring(2, 18)));
			s.sy4.setSystemtype(To_ASCII(str.substring(18, 50)));
			s.sy4.setSystemver(To_ASCII(str.substring(50, 82)));
			s.sy4.setGgroup(To_ASCII(str.substring(82, 162)));
			s.sy4.setPn(To_ASCII(str.substring(162, 192)));
			
			s.sy4.setPd(Integer.valueOf(Dao_string(str.substring(192, 200))));
			s.sy4.setPs(Integer.valueOf(Dao_string(str.substring(200, 208))));
			s.sy4.setPl(Integer.valueOf(Dao_string(str.substring(208, 216))));
			
			s.sy4.setS_loadcurrent(Dao_string(str.substring(216, 224)));
			...
			s.sy4.setAxis_loadcurrent(axis_loadcurrent);
			
			s.sy4.setOntime(Dao_string(str.substring(248, 256)));
			s.sy4.setRuntime(Dao_string(str.substring(256, 264)));
			s.sy4.setTimestamp(To_ASCII(str.substring(264, 312)));
...
//						System.out.print(hex.toUpperCase());//转换成大写
					}
//					System.out.println();
					time = new String(buf);
...
//			in.read(sn);
//			System.out.println("Q:"+Arrays.toString(sn));
