...
public final class JsonUtil
{
	...
	public static String object2Json(final Object bean)
    {
            ...
            final Method[] methods = bean.getClass().getMethods(); //反射，得到方法数组
            final StringBuilder sb = new StringBuilder(methods.length << 4); //4次方方法数的初始容量
            ...
            for (final Method method : methods)
            {
                    try
                    {
                            final String name = method.getName();//得到方法名
                            String key = "";
                            if (name.startsWith("get") && !name.startsWith("getClass"))//得到get方法
                            {
                                    key = name.substring(3);//获取属性名，在下面要对属性名进行处理
                                    //防死循环，这是什么意思？？？
...
//                                        }
                            ...
                            else if (name.startsWith("is"))//这个有用吗？暂留
                            {
                            ...
                            {
                                    if (key.length() == 1)//如果属性只有一个字母
                                    {
                                            key = key.toLowerCase(); //转成小写字母
                                    }
                                    else if (!Character.isUpperCase(key.charAt(1)))//如果属性的第二个字母不是大写
                                    {//那么要将第一个字母转为小写
                                        key = key.substring(0, 1).toLowerCase() + key.substring(1);
                                    }
                                    final Object elementObj = method.invoke(bean);//传入的object必须得是bean
            ...
            if (sb.length() == 1)//说明bean中没有任何属性方法，那么这个bean有可能是一个基本数据类型的对象
            {
